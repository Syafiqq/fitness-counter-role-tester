...
class SitUp: IdentifiableFragment()
{
    ...
    override fun doSend(v: View?)
    {
        ...
        if (event.presetActive != null)
        {
            ...
            else
            {
                ...
                PresetHelper.saveSitUp(event.presetActive!!, this.edittext_participant.text.toString().toInt(), this.sitUp, DatabaseReference.CompletionListener { error, _ ->
                    run {
                        with(this@SitUp)
                        {
                            if (error == null)
    ...
    private fun countStopwatch(view: View? = null)
    {
        ...
        this.stopwatchService?.run {
            with(this@SitUp)
            {
                if ((org.joda.time.DateTime.now().millis - this@run.getStopwatch().startedAt.millis <= this@SitUp.limit))
                {
